...
let execFileSync: typeof nodeExecFileSync = nodeExecFileSync;
let fork: typeof nodeFork = nodeFork;
let spawn: typeof nodeSpawn = nodeSpawn;
...
function observeWindowsWatcherMarker(event: WindowsProcessStartEvent): void {
  ...
  if (
    ...
    event.eventTime !== marker.birth
  ) return;
  const failureTrigger = process.env.ALOOK_APP_TEST_WINDOWS_WATCHER_FAILURE_TRIGGER;
  const withhold = process.env.ALOOK_APP_TEST_WINDOWS_WATCHER_FAILURE === "withhold-marker" &&
    !!failureTrigger && existsSync(failureTrigger);
  if (withhold) return;
...
async function flushWindowsProcessWatcher(deadline = Date.now() + 5_000): Promise<void> {
  ...
  const failureTrigger = process.env.ALOOK_APP_TEST_WINDOWS_WATCHER_FAILURE_TRIGGER;
  if (
    process.env.ALOOK_APP_TEST_WINDOWS_WATCHER_FAILURE === "exit" &&
    failureTrigger && existsSync(failureTrigger)
  ) {
  ...
  try {
    ...
    while (!markerRecord) {
      ...
      if (Date.now() >= deadline) throw new Error("Windows process watcher did not observe the required marker");
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const poll = () => {
        ...
        if (Date.now() >= deadline) return reject(new Error("Windows process watcher did not observe the required marker"));
        setTimeout(poll, 10);
      };
      poll();
    });
...
export interface ServiceSupervisorRuntime {
  ...
  testing: {
    ...
    waitForTreeSignalBarrier(deadline: number): Promise<void>;
...
function resetRuntimeState(): void {
  ...
  server = undefined;
  child = undefined;
  commandRootPid = undefined;
  ownedWindowsProcesses.clear();
  windowsSeedIdentitiesLocked = false;
  windowsProcessWatcher = undefined;
  windowsProcessWatcherError = undefined;
  windowsProcessWatcherStartedAt = 0n;
  windowsLastProcessStartEvent = 0n;
  windowsSeedCutoff = 0n;
  pendingWindowsStartEvents.length = 0;
  latestWindowsStartEvents.clear();
  windowsWatcherMarker = undefined;
  treeSignalDelayConsumed = false;
  if (heartbeat) clearInterval(heartbeat);
  heartbeat = undefined;
  status = {
    ok: false,
...
export function createServiceSupervisorRuntime(
  ...
  process = adapters.process ?? globalThis.process;
  execFileSync = adapters.execFileSync ?? nodeExecFileSync;
  fork = adapters.fork ?? nodeFork;
  spawn = adapters.spawn ?? nodeSpawn;
  resetRuntimeState();
