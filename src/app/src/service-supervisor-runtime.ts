...
type ManagedChild = ReturnType<typeof nodeSpawn> & EventEmitter;
...
interface WindowsWatcherMarker {
  child: ManagedChild;
  pid: number;
  parentPid: number;
  birth: bigint;
...
export interface ServiceSupervisorRuntimeAdapters {
  ...
  clearScheduledInterval: typeof globalThis.clearInterval;
  scheduleImmediate: typeof globalThis.setImmediate;
...
export interface ServiceSupervisorRuntime {
  run(): void;
  testing: {
    tokenMatches(candidate: unknown): boolean;
    pidAlive(pid: number): boolean;
    posixTreeAlive(pid: number): boolean;
    startWindowsProcessWatcher(deadline: number): Promise<void>;
    recordWindowsStartEvent(value: unknown): void;
    processWindowsStartEvent(event: WindowsProcessStartEvent): void;
    flushWindowsProcessWatcher(deadline?: number): Promise<void>;
    refreshOwnedWindowsProcesses(deadline?: number, snapshot?: WindowsProcessRecord[]): Promise<WindowsProcessRecord[]>;
    lockOwnedWindowsSeeds(runnerPid: number, serviceRootPid: number, deadline: number): Promise<void>;
    windowsOperationTimeout(deadline?: number): number;
    windowsProcessSnapshot(deadline?: number): WindowsProcessRecord[];
    ownedTreeAlive(pid: number, deadline?: number): Promise<boolean>;
    signalTree(pid: number, force: boolean, deadline: number): Promise<unknown>;
    waitForTreeExit(pid: number, timeoutMs: number, terminationDeadline: number): Promise<boolean>;
    terminateOwnedTree(): Promise<AuthorityStatus>;
    startControlServer(): Promise<void>;
    setState(value: {
      init?: SupervisorInit;
      child?: ManagedChild;
      status?: AuthorityStatus;
      commandRootPid?: number;
      windowsSeedIdentitiesLocked?: boolean;
      windowsProcessWatcher?: ManagedChild;
      windowsProcessWatcherError?: Error;
      windowsProcessWatcherStartedAt?: bigint;
      windowsSeedCutoff?: bigint;
      ownedWindowsProcesses?: WindowsProcessRecord[];
...
export function createServiceSupervisorRuntime(
  adapters: Partial<ServiceSupervisorRuntimeAdapters> = {},
): ServiceSupervisorRuntime {
  const process = adapters.process ?? globalThis.process;
  const execFileSync = adapters.execFileSync ?? nodeExecFileSync;
  const fork = adapters.fork ?? nodeFork;
  const spawn = adapters.spawn ?? nodeSpawn;
  const now = adapters.now ?? (() => Date.now());
  const scheduleTimeout = adapters.scheduleTimeout ?? globalThis.setTimeout;
  const clearScheduledTimeout = adapters.clearScheduledTimeout ?? globalThis.clearTimeout;
  const scheduleInterval = adapters.scheduleInterval ?? globalThis.setInterval;
  const clearScheduledInterval = adapters.clearScheduledInterval ?? globalThis.clearInterval;
  const scheduleImmediate = adapters.scheduleImmediate ?? globalThis.setImmediate;
  const existsSync = adapters.existsSync ?? nodeExistsSync;
  ...
  const readWindowsProcessSnapshot = adapters.readWindowsProcessSnapshot ?? ((timeoutMs) => {
    const script = [
      "$ErrorActionPreference='Stop'",
  ...
  let windowsLastProcessStartEvent = 0n;
  ...
  function windowsFileTimeNow(): bigint {
    return (BigInt(now()) * 10_000n) + 116_444_736_000_000_000n;
  }
  ...
  function observeWindowsWatcherMarker(event: WindowsProcessStartEvent): void {
    const marker = windowsWatcherMarker;
    if (
      !marker ||
      event.pid !== marker.pid ||
      event.parentPid !== marker.parentPid ||
      event.eventTime < marker.notBefore
    ) return;
  ...
  function recordWindowsStartEvent(value: unknown): void {
    const event = value as { pid?: unknown; parentPid?: unknown; eventTime?: unknown };
    ...
    if (eventTime < windowsLastProcessStartEvent) {
      windowsProcessWatcherError = new Error("Windows process watcher creation time regressed");
      return;
    }
    windowsLastProcessStartEvent = eventTime;
    ...
    observeWindowsWatcherMarker(record);
  ...
  async function startWindowsProcessWatcher(deadline: number): Promise<void> {
    const watcher = spawnWindowsProcessWatcher(process.pid);
    windowsProcessWatcher = watcher;
    let stdout = "";
    let stderr = "";
    await new Promise<void>((resolve, reject) => {
      let ready = false;
      const timeoutMs = Math.max(1, Math.min(WINDOWS_TREE_COMMAND_TIMEOUT_MS, deadline - now()));
      const timer = scheduleTimeout(() => reject(new Error("Windows process watcher did not become ready")), timeoutMs);
  ...
  async function flushWindowsProcessWatcher(deadline = now() + 5_000): Promise<void> {
    ...
    const notBefore = windowsFileTimeNow();
    const markerChild = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)"], {
      stdio: "ignore",
      windowsHide: true,
    }) as ManagedChild;
    try {
      ...
      while (!markerRecord) {
        if (windowsProcessWatcherError) throw windowsProcessWatcherError;
        if (!windowsProcessWatcher?.pid) throw new Error("Windows process watcher authority is missing");
        if (markerChild.exitCode !== null || markerChild.signalCode !== null) {
          throw new Error("Windows process watcher marker exited before observation");
        }
        if (now() >= deadline) throw new Error("Windows process watcher did not observe the required marker");
        markerRecord = windowsProcessSnapshot(deadline).find((record) => record.pid === markerChild.pid);
        if (markerRecord) break;
        await new Promise((resolve) => scheduleTimeout(resolve, 10));
      }
      if (markerRecord.parentPid !== process.pid) {
        throw new Error("Windows process watcher marker parent identity changed");
      }
      if (BigInt(markerRecord.birth) < notBefore) {
        throw new Error("Windows process watcher marker identity predates its causal barrier");
      }
      await new Promise<void>((resolve, reject) => {
        const marker: WindowsWatcherMarker = {
          child: markerChild,
          pid: markerRecord!.pid,
          parentPid: markerRecord!.parentPid,
          birth: BigInt(markerRecord!.birth),
          notBefore,
          observed: false,
          resolve,
        };
        windowsWatcherMarker = marker;
        const alreadyObserved = latestWindowsStartEvents.get(marker.pid) ??
          pendingWindowsStartEvents.find((event) => event.pid === marker.pid);
        if (alreadyObserved) observeWindowsWatcherMarker(alreadyObserved);
        const poll = () => {
          if (marker.observed) return;
          if (windowsProcessWatcherError) return reject(windowsProcessWatcherError);
          if (!windowsProcessWatcher?.pid) return reject(new Error("Windows process watcher authority is missing"));
          if (now() >= deadline) return reject(new Error("Windows process watcher did not observe the required marker"));
          scheduleTimeout(poll, 10);
        };
        poll();
      });
      if (markerChild.exitCode !== null || markerChild.signalCode !== null) {
        throw new Error("Windows process watcher marker exited before identity confirmation");
      }
      const confirmed = windowsProcessSnapshot(deadline).find((record) => record.pid === markerRecord!.pid);
      if (
        !confirmed ||
        confirmed.parentPid !== markerRecord.parentPid ||
        confirmed.birth !== markerRecord.birth
      ) {
  ...
  function windowsProcessSnapshot(deadline?: number): WindowsProcessRecord[] {
    const output = readWindowsProcessSnapshot(windowsOperationTimeout(deadline));
    if (!output) return [];
  ...
  async function refreshOwnedWindowsProcesses(
    ...
    await flushWindowsProcessWatcher(deadline);
  ...
  async function signalTree(pid: number, force: boolean, deadline: number): Promise<unknown> {
    try {
      ...
      if (force) signalProcess(-pid, "SIGKILL");
      else signalProcess(pid, "SIGTERM");
      return undefined;
    } catch (error) {
      return error;
    }
  ...
  async function waitForTreeExit(pid: number, timeoutMs: number, terminationDeadline: number): Promise<boolean> {
    const deadline = Math.min(terminationDeadline, now() + timeoutMs);
    if (process.platform === "win32") {
      while (now() < deadline) {
  ...
  async function terminateOwnedTree(): Promise<AuthorityStatus> {
    ...
    const deadline = now() + TERMINATION_BUDGET_MS;
  ...
  async function start(nextInit: SupervisorInit): Promise<void> {
    ...
    if (init.mode === "reservation") {
      const beat = () => writeFileSync(init!.mode === "reservation" ? init!.heartbeatPath : "", String(now()), { mode: 0o600 });
      beat();
      heartbeat = scheduleInterval(beat, 1_000);
      heartbeat.unref();
      emit({ type: "acquired", status });
      return;
    }

    const acquisitionDeadline = now() + SUPERVISOR_ACQUISITION_BUDGET_MS;
    ...
    commandRootPid = await new Promise<number>((resolve, reject) => {
      const timeoutMs = Math.max(1, Math.min(WINDOWS_TREE_COMMAND_TIMEOUT_MS, acquisitionDeadline - now()));
      const timer = scheduleTimeout(() => reject(new Error(`${init?.service ?? "service"} command runner did not start`)), timeoutMs);
  ...
  function startCommandRunner(runner: CommandRunnerInit): void {
    const invocation = process.platform === "win32"
      ? createWindowsCommandInvocation(runner.command, runner.args, runner.env)
      : { command: runner.command, args: runner.args };
    const command = spawn(invocation.command, invocation.args, {
      cwd: runner.cwd,
      detached: false,
      stdio: "inherit",
      env: runner.env,
  ...
  function installServiceSupervisorHandlers(): void {
    ...
    const setState: ServiceSupervisorRuntime["testing"]["setState"] = (value) => {
      if (value.init !== undefined) init = value.init;
      if (value.child !== undefined) child = value.child;
      if (value.status !== undefined) status = value.status;
      if (value.commandRootPid !== undefined) commandRootPid = value.commandRootPid;
      if (value.windowsSeedIdentitiesLocked !== undefined) windowsSeedIdentitiesLocked = value.windowsSeedIdentitiesLocked;
      if (value.windowsProcessWatcher !== undefined) windowsProcessWatcher = value.windowsProcessWatcher;
      if (value.windowsProcessWatcherError !== undefined) windowsProcessWatcherError = value.windowsProcessWatcherError;
      if (value.windowsProcessWatcherStartedAt !== undefined) windowsProcessWatcherStartedAt = value.windowsProcessWatcherStartedAt;
      if (value.windowsSeedCutoff !== undefined) windowsSeedCutoff = value.windowsSeedCutoff;
      if (value.ownedWindowsProcesses !== undefined) {
        ownedWindowsProcesses.clear();
        for (const record of value.ownedWindowsProcesses) ownedWindowsProcesses.set(record.pid, record);
      }
      if (value.latestWindowsStartEvents !== undefined) {
        latestWindowsStartEvents.clear();
        for (const event of value.latestWindowsStartEvents) latestWindowsStartEvents.set(event.pid, event);
      }
      if (value.heartbeat !== undefined) heartbeat = value.heartbeat;
    };
    return {
      run: installServiceSupervisorHandlers,
      testing: {
        tokenMatches,
        pidAlive,
        posixTreeAlive,
        startWindowsProcessWatcher,
        recordWindowsStartEvent,
        processWindowsStartEvent,
        flushWindowsProcessWatcher,
        refreshOwnedWindowsProcesses,
        lockOwnedWindowsSeeds,
        windowsOperationTimeout,
...
export function runServiceSupervisor(): void {
  createServiceSupervisorRuntime().run();
}
