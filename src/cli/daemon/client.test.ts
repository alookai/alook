...
// ---------------------------------------------------------------------------
// DaemonClient.checkStandby() tests
// ---------------------------------------------------------------------------

describe("DaemonClient.checkStandby() with mocked fetch", () => {
  const originalFetch = globalThis.fetch;
  ...
  it("returns standby: true when no workspace bound", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ runtimes: [], standby: true }),
    }) as unknown as typeof fetch;

    const client = new DaemonClient("http://localhost:8080");
    const resp = await client.checkStandby("tok", {
      daemon_id: "d1",
      device_name: "mac",
      cli_version: "1.0",
      workspaces_root: "/tmp",
    ...
    expect(resp.standby).toBe(true);
    expect(resp.runtimes).toEqual([]);
  });

  it("returns workspace data when token has been bound", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
    ...
    expect(resp.standby).toBeUndefined();
    expect(resp.runtimes).toHaveLength(1);
    expect(resp.workspaceId).toBe("sp_ws1");
  });

  it("calls /api/daemon/register endpoint", async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
      json: () => Promise.resolve({ runtimes: [], standby: true }),
    ...
    const client = new DaemonClient("http://localhost:8080");
    await client.checkStandby("my_token", {
      daemon_id: "d1",
      device_name: "mac",
      cli_version: "1.0",
      workspaces_root: "/tmp",
    ...
    expect(globalThis.fetch).toHaveBeenCalledWith(
      "http://localhost:8080/api/daemon/register",
      expect.objectContaining({
        method: "POST",
        headers: expect.objectContaining({
