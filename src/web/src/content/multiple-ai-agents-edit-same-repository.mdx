...
**Alook keeps agent roles, threads, and task assignments visible while the agents continue to run on your machine.** A path scope such as `src/auth/` can stay in the task record instead of living only in a temporary prompt. Review can move to another agent, while merge approval stays with you.

You can self-host Alook from [GitHub](https://github.com/alookai/alook), or register at [alook.ai](https://alook.ai) and connect your local runtime. **Alook helps show who is assigned to the work. Git still records and merges the code.** It does not resolve merge conflicts or replace your editor.
