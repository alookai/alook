...
describe("server rail PDD component contract", () => {
  ...
  it("separates touch scroll intent and exposes keyboard drag instructions", () => {
    ...
    expect(adapter).toContain('element.addEventListener("touchstart", suppressTouchMenu, { passive: true })')
    expect(adapter).toContain('dragHandle.addEventListener("touchstart", suppressTouchMenu, { passive: true })')
    expect(adapter).toContain("event.stopPropagation()")
    ...
    expect(server).toContain("[-webkit-touch-callout:none]")
    expect(folder).toContain("[-webkit-touch-callout:none]")
    expect(server).toContain("data-rail-drag-preview")
    expect(folder).toContain("data-rail-drag-preview")
    expect(adapter).toContain("data-rail-floating-preview")
