...
const renderers: ReturnType<typeof rtlRender>[] = []
...
async function renderHook(overrides: Partial<HookOptions> = {}) {
  ...
  await act(async () => {
    ...
    }), { container: rtlContainer, baseElement: rtlContainer })
  })
  ...
  return {
    ...
    async rerender(next: Partial<HookOptions>) {
      ...
      await act(async () => renderer.rerender(createElement(Capture, {
        options,
        onResult: (api) => { current = api },
      })))
    },
...
function register(
  ...
  const dispose = api.registerItem(
    entity,
    element as unknown as HTMLElement,
    handle as unknown as HTMLElement,
  )
  return { entity, element, handle, dragPreview, dispose }
}
