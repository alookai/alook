...
vi.mock("@/components/community/shell/community-panel", () => ({
  CommunityPanel: vi.fn(() => React.createElement("community-panel")),
}))
...
describe("ForumChannelSurface ownership", () => {
  ...
  it("owns forum create, tag, delete, permissions, and pending wiring without an active message feed", async () => {
    ...
    act(() => {
      renderer = render(React.createElement(ForumChannelSurface, props))
    })
  ...
  it("allows server managers to edit and delete other authors' posts", () => {
    act(() => {
      render(React.createElement(ForumChannelSurface, surfaceProps({ viewerRole: "admin" })))
    })
    const forumProps = mockedForumSurface.mock.calls.at(-1)![0]
  ...
  it("returns a rejected tag mutation to the editor so its draft stays retryable", async () => {
    ...
    act(() => {
      render(React.createElement(ForumChannelSurface, surfaceProps()))
    })
    const forumProps = mockedForumSurface.mock.calls.at(-1)![0]
    const post = {
      id: "post_1",
  ...
  it("resets the right panel when channelId changes without relying on an outer remount", () => {
    ...
    let renderer: ReturnType<typeof render>
    act(() => {
      renderer = render(React.createElement(ForumChannelSurface, firstProps))
    })
    act(() => {
    ...
    expect(renderer!.container.querySelectorAll("community-panel")).toHaveLength(0)
  })
})
