...
vi.mock("@/components/community/channels/thread-split-view", () => ({
  ThreadSplitView: ({
    ...
    split && React.createElement(
      "section",
      { "data-testid": "community-thread-split-parent" },
      parent,
    ),
    React.createElement(
      "section",
      { "data-testid": "community-thread-split-panel" },
      thread,
    ),
  ),
}))
...
describe("ChannelRoute message surface ownership", () => {
  ...
  it("mounts authoritative split geometry before a thread body is active", () => {
    ...
    expect(screen.getByTestId("community-thread-split")).toHaveAttribute(
      "data-layout",
      "split",
    )
  ...
  it("composes a wide child route as parent + thread with fullscreen and close actions", () => {
    ...
    fireEvent.click(screen.getByTestId("community-thread-split-close"))
