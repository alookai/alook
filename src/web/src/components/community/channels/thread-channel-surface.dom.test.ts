...
describe("ThreadChannelSurface ownership", () => {
  ...
  it("installs the parent-opener claim hook before initializing the child feed", () => {
    ...
    act(() => {
      render(renderSurface({ threadOpenerHandoff: handoff, parentIsForum: true }))
    })
  ...
  it("owns the child feed and preserves opener, message-list, and composer wiring", () => {
    ...
    act(() => {
      render(React.createElement(ThreadChannelSurface, props))
    })
  ...
  it("keeps authoritative thread chrome mounted across body loading and error", () => {
    ...
    let renderer!: ReturnType<typeof render>
    act(() => {
      renderer = render(React.createElement(ThreadChannelSurface, props))
    })
    ...
    act(() => renderer.rerender(React.createElement(ThreadChannelSurface, props)))
  ...
  it("preserves regular-thread identity, rename, panel, and dialog wiring", async () => {
    ...
    expect(renderer!.container.querySelector('[data-manage-dialog="true"]')).toBeInTheDocument()
  })

  it("uses the forum opener-message rename path only for the post creator", async () => {
    act(() => {
      render(renderSurface({ parentIsForum: true, channelName: "Post title" }))
    })
    let headerProps = mockedChannelHeader.mock.calls.at(-1)![0]
    expect(headerProps).toEqual(expect.objectContaining({
      channel: "Post title",
    ...
    act(() => {
      render(renderSurface({ parentIsForum: true, childCreatorId: "other_user" }))
    })
    headerProps = mockedChannelHeader.mock.calls.at(-1)![0]
  ...
  it("preserves rename errors for regular threads and forum posts", async () => {
    ...
    act(() => {
      render(renderSurface({ parentIsForum: true }))
    })
