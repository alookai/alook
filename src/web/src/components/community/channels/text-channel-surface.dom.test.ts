...
let latestController: MessageChannelControllerValue
...
describe("MessageChannelController scroll target ownership", () => {
  ...
  it("keeps a loaded target until MessageList reports consumption", () => {
    ...
    expect(renderer!.container.querySelector("div")).toHaveAttribute("data-scroll-target", "m_target")
    fireEvent.click(renderer!.container.querySelector("div")!)
  ...
  it("keeps a missing target across warm cache until the anchor request errors", () => {
    ...
    act(() => renderer!.rerender(renderController(messageFeed)))
    expect(renderer!.container.querySelector("div")).not.toHaveAttribute("data-scroll-target")
  })
  ...
  it("keeps message action references stable while calling the latest surface callbacks", async () => {
    ...
    const firstActions = latestController.messageActions

    act(() => {
      renderer!.rerender(renderController(
        feed({ messages: [{ id: "m_target", content: "latest" }] }),
        "m_target",
        { onOpenThread: latestOpenThread, onOpenPinned: latestOpenPinned },
      ))
    })
    const latestActions = latestController.messageActions
  ...
  it("retries a failed channel upload with the identical thumbnail Blob in multipart", async () => {
    ...
    act(() => {
      renderer!.rerender(renderController(feed({
        messages: [{ id: "temp_1", clientNonce: "nonce_1", type: "chat", content: "photo" }],
      })))
    })
    const retriedController = latestController
    await act(async () => {
