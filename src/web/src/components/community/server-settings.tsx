...
export function ServerSettings({
  section, setSection, onClose, serverName, serverDescription, members, invites, auditLog, onOpenProfile,
  ...
  return (
    ...
    >
      ...
      <nav className="flex w-60 shrink-0 flex-col gap-2 overflow-y-auto thin-scrollbar border-r border-border p-3" style={{ background: "var(--d-rail)" }}>
        ...
        <Button variant="destructive" size="sm" className="justify-start" onClick={onDeleteServer}><Trash2 className="size-4" /> Delete Server</Button>
      </nav>
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-background">
        ...
        <div className="flex-1 overflow-y-auto thin-scrollbar p-5">
          <TabsContent value="overview"><SettingsOverview serverName={serverName} serverDescription={serverDescription} onUploadIcon={onUploadIcon} onUpdateServer={onUpdateServer} /></TabsContent>
...
function SettingsOverview({ serverName, serverDescription, onUploadIcon, onUpdateServer }: { serverName: string; serverDescription?: string; onUploadIcon?: () => void; onUpdateServer?: (name: string, desc: string) => void }) {
