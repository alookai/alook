...
describe("MediaAttachmentBlock", () => {
  ...
  it("mounts compact audio controls and keeps mobile actions at least 44px", () => {
    ...
    const collapse = renderer!.root.findByProps({ "data-testid": tid.mediaCollapse("voice.ogg") })
    expect(collapse.props.className).toContain("size-11")
    expect(collapse.props.className).toContain("text-muted-foreground")
    const download = renderer!.root.findByProps({ "data-testid": tid.mediaDownload("voice.ogg") })
    expect(download.props.className).toContain("size-11")
    expect(download.props.className).toContain("text-muted-foreground")
  })
