...
let controllerView: ReturnType<typeof useComposerController>
...
const Harness = forwardRef<ComposerHandle, ComposerProps>(
  function Harness(props, ref) {
    ...
    controllerView = view
    return createElement("div", { "data-controller-probe": "" })
  },
)
...
describe("useComposerController", () => {
  ...
  it("forwards the optional channel source into suggestions and its presentation into the view", async () => {
    ...
    expect(controllerView).toMatchObject({
      channelRefPresentation: { status: "loading" },
    })
  })
  ...
  it("retains everything after false/throw, unlocks, and clears only on accepted true", async () => {
    ...
    const renderer = render(createElement(Harness, acceptedProps(reject)))
    await enter()
    ...
    renderer.rerender(createElement(Harness, acceptedProps(throws)))
    await enter()
    ...
    renderer.rerender(createElement(Harness, acceptedProps(accept)))
    await enter()
  ...
  it("awaits deferred resolve/reject without Composer-owned cleanup", async () => {
    ...
    const renderer = render(createElement(Harness, props))
    await enter(true)
    await enter(true)
  ...
  it("reads popup refs at keydown time and preserves handle/paste semantics", async () => {
    ...
    render(createElement(Harness, { ...acceptedProps(accept), ref: handleRef }))
  ...
  it("skips long-paste names already used by pending files", async () => {
    ...
    renderer.rerender(createElement(Harness, props))
  ...
  it("retains one TipTap editor instance while render-time configuration reruns", async () => {
    const accept = vi.fn(() => true)
    const renderer = render(createElement(Harness, acceptedProps(accept)))
    const firstEditor = controllerView.editor
    renderer.rerender(
    ...
    const secondEditor = controllerView.editor
  ...
  it("keeps one live input-capability contract across coarse and hover-capable inputs", async () => {
    ...
    expect(controllerView.showSend).toBe(true)
    ...
    expect(controllerView.showSend).toBe(false)
  ...
  it("updates the live placeholder without replacing or resetting the editor", async () => {
    ...
    expect(controllerView.editor).toBe(firstEditor)
  ...
  it("skips all draft restore and chat focus effects in forum mode", async () => {
    ...
    let renderer!: ReturnType<typeof render>
    await act(async () => {
      renderer = render(createElement(Harness, props))
    })
  ...
  it("preserves the full key priority matrix and text parser pipeline", async () => {
    ...
    await act(async () => {
      renderer.rerender(createElement(Harness, forumProps))
    })
  ...
  it("pins the layout/draft/typing ordering boundaries in source", () => {
    const source = readFileSync(
      resolve(
        ...
        "src/components/community/messages/use-composer-controller.ts",
      ),
      "utf8",
    )
