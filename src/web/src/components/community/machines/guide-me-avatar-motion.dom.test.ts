...
describe("first-signup guide avatar motion", () => {
  ...
  it("exposes stable QA targets for the travelling avatar and exact orbit landing point", async () => {
    render(React.createElement(GuideMeAvatarMotion, {
    ...
    expect(screen.getByTestId(tid.machineFirstSignupGuide)).toBeInTheDocument()
    expect(screen.getByTestId(tid.machineGuideAvatarTarget)).toBeInTheDocument()
  })
})
