...
function RuntimeChip({ runtime }: { runtime: CommunityMachineRuntime }) {
  const chip = (
    <span className="inline-flex max-w-[160px] items-center gap-2 rounded-md border border-border bg-card px-2 py-1 text-[11px]">
      ...
      <TooltipTrigger
        render={
          <button
            ...
            className="inline-flex max-w-[160px] items-center gap-2 rounded-md border border-border bg-card px-2 py-1 text-[11px] transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <ProviderLogo provider={runtime.id} className="size-3.5 shrink-0" />
            <span className="truncate font-medium text-foreground">{runtime.id}</span>
          </button>
