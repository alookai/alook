...
//
// Mock the sheet shell and heavy children to render passthrough, and mock the data/mutation
...
function radios(renderer: ReturnType<typeof rtlRender>, name: string): Radio[] {
  return [...renderer.container.querySelectorAll<HTMLInputElement>(`input[name="${name}"]`)]
    .map((input) => ({ name: input.name, value: input.value, checked: input.checked }))
}

function checkedValue(renderer: ReturnType<typeof rtlRender>, name: string): string | null {
...
describe("CreateBotSheet — auto-select defaults", () => {
  ...
  it("resets the model to Default (null) when the user switches machine/runtime", () => {
    ...
    const serverRadio = renderer.container.querySelector<HTMLInputElement>(
      'input[name="bot-machine"][value="server"]',
    )!
  ...
  it("passes only the selected machine's same-runtime catalog to ModelField", () => {
    ...
    const machineB = renderer.container.querySelector<HTMLInputElement>(
      'input[name="bot-machine"][value="machine-b"]',
    )!
    fireEvent.click(machineB)
    expect(modelFieldRenders.at(-1)?.runtime?.reasoning).toEqual(catalog("b-model"))

    const machineA = renderer.container.querySelector<HTMLInputElement>(
      'input[name="bot-machine"][value="machine-a"]',
    )!
