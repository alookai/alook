...
function render(days: BotActivityDay[]) {
  return renderDom(React.createElement(BotActivityHeatmap, { days }))
}

// The cell spans in DOM order — the grid renders exactly 30, oldest→newest.
function cells(r: ReturnType<typeof renderDom>) {
  return [...r.container.querySelectorAll<HTMLElement>('[data-cell="true"]')]
}

// Each cell's tooltip text, in DOM order — joined across its lines.
function tips(r: ReturnType<typeof renderDom>): string[] {
  return [...r.container.querySelectorAll<HTMLElement>('[data-tip="true"]')]
...
describe("BotActivityHeatmap — calendar-axis fill", () => {
  ...
  it("colors by ABSOLUTE SENT count per day (busier bots read darker), across fixed bands", () => {
    ...
    const tokens = cells(r).map((cell) => [...cell.classList])
  ...
  it("colors by sent, not handled — a handled-heavy / sent-light day stays pale", () => {
    ...
    expect([...cell.classList]).toContain("bg-status-online/30") // b1 by sent=3
