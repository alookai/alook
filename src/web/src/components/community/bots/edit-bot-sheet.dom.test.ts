...
function renderSheet(bot: BotSummary = BOT) {
  return render(React.createElement(EditBotSheet, { bot, open: true, onOpenChange: vi.fn() }))
}

function saveButton(renderer: ReturnType<typeof render>) {
  return renderer.getByRole("button", { name: "Save" })
}

function pickRuntime(renderer: ReturnType<typeof render>, value: string) {
  fireEvent.click(renderer.container.querySelector<HTMLInputElement>(
    `input[name="edit-bot-runtime"][value="${value}"]`,
  )!)
}
...
function runtimeRadio(renderer: ReturnType<typeof render>, value: string) {
  return renderer.container.querySelector<HTMLInputElement>(
    `input[name="edit-bot-runtime"][value="${value}"]`,
  )!
}
...
describe("EditBotSheet — model switch toast (online-only)", () => {
  ...
  it("PATCH success → 'Model switch to <new> dispatched'", async () => {
    ...
    expect(renderer.getByTestId("bot-provider-picker")).toBeInTheDocument()
...
describe("EditBotSheet — reasoning effort", () => {
  ...
  it("preserves stored Low on an old daemon when saving an unrelated field", async () => {
    ...
    fireEvent.click(renderer.getByTestId("set-description"))
    act(() => {
      fireEvent.click(saveButton(renderer))
    })
    await flush()
...
describe("EditBotSheet — provider switch AlertDialog + toast", () => {
  ...
  it("Save with new provider opens confirm dialog and does not PATCH yet", () => {
    ...
    expect(renderer.getAllByTestId("provider-confirm")).toHaveLength(1)
  ...
  it("confirm → PATCH with runtime + cleared model, then dispatched toast", async () => {
    ...
    fireEvent.click(renderer.getByTestId("provider-confirm-action"))
    await flush()
    expect(updateMutateAsync).toHaveBeenCalledWith(
  ...
  it("cancel dialog → no PATCH", () => {
    ...
    expect(renderer.getByTestId("provider-cancel")).toBeInTheDocument()
    expect(updateMutateAsync).not.toHaveBeenCalled()
  })
})
...
describe("EditBotSheet — current unhealthy runtime stays selectable (Alli #1287 / Ruthann #1289)", () => {
  ...
  it("selected current runtime is not disabled even when unhealthy; other unhealthy candidates are", () => {
    ...
    expect(otherUnhealthy.disabled).toBe(true)
  ...
  it("shows a stored removed runtime as unavailable and allows switching without changing bot data first", () => {
    ...
    expect(renderer.getAllByText("unavailable", { selector: "span" })).toHaveLength(1)
    expect(healthy.disabled).toBe(false)

    fireEvent.click(healthy)
    expect(runtimeRadio(renderer, "codex").checked).toBe(true)
    expect(updateMutateAsync).not.toHaveBeenCalled()
  })
})
