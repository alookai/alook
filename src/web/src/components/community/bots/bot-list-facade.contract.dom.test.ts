...
vi.mock("./bot-list-view", () => ({
  renderBotListView: (props: unknown, controller: unknown) => {
    ...
    return React.createElement("div", { "data-testid": "bot-list-view" })
  },
}))
...
describe("BotList facade contract", () => {
  ...
  it("invokes the controller and plain view exactly once with omitted props resolved", () => {
    const rendered = render(React.createElement(facade.BotList))
    expect(screen.getByTestId("bot-list-view")).toBeInTheDocument()
    ...
    const onBack = vi.fn()
    rendered.rerender(React.createElement(facade.BotList, { onBack }))
  ...
  it("keeps the production page and existing regression on the original path", () => {
    ...
    const existingTest = source("src/components/community/bots/bot-list-card.dom.test.ts")
