...
function findInterruptButton(container: HTMLElement) {
  return container.querySelector<HTMLButtonElement>(
    'button[aria-label="Stop current agent turn"], button[aria-label="Stopping current agent turn"]',
  )
}
...
describe("ProfileCard contextual metadata", () => {
  ...
  it("sends once and enters a disabled loading state until exact Idle arrives", async () => {
    ...
    const renderer = render(interactiveProfile(
      queryClient,
      { emoji: "⚡", text: "Working on it" },
    ))

    const stop = findInterruptButton(renderer.container)
    ...
    renderer.rerender(interactiveProfile(
      queryClient,
    ...
    renderer.rerender(interactiveProfile(queryClient, { emoji: "💤", text: "Idle" }))
    expect(findInterruptButton(renderer.container)).toBeNull()

    renderer.rerender(interactiveProfile(
      queryClient,
      { emoji: "⚡", text: "Working on it" },
    ))
    expect(findInterruptButton(renderer.container)?.disabled).toBe(false)
    renderer.unmount()
  })
  ...
  it("fails open after 10 seconds while the activity is still running", async () => {
    ...
    expect(findInterruptButton(renderer.container)?.disabled).toBe(false)
    expect(renderer.container.textContent).toContain("Stop")
    renderer.unmount()
  })
