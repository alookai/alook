...
const captures = vi.hoisted(() => ({ root: vi.fn(), trigger: vi.fn() }))
  ...
  return {
    ContextMenu: {
      Root: ({ children, ...props }: { children?: ReactNode }) => {
        const child = children as ReactElement<{ render?: ReactElement<{ id?: string }> }> | undefined
        captures.root({ ...props, __id: child?.props.render?.props.id ?? "trigger" })
        return ReactModule.createElement("div", { "data-context-root": "" }, children)
      },
      Trigger: ({ render, children, ...props }: { render?: ReactElement; children?: ReactNode }) => {
        captures.trigger({ ...props, __id: render?.props.id ?? "trigger" })
        return ReactModule.cloneElement(render ?? ReactModule.createElement("div"), props, children)
      },
...
function capturedNode(capture: ReturnType<typeof vi.fn>, id = "trigger") {
  return {
    get props() {
      const props = capture.mock.calls
        .toReversed()
        .map(([value]) => value as Record<string, unknown>)
        .find((value) => value.__id === id)
      if (!props) throw new Error(`Expected captured props for ${id}`)
      return props as Record<string, unknown> & {
        disabled?: boolean
        onContextMenu: (event: unknown) => void
        onKeyDown: (event: unknown) => void
        onPointerDownCapture: (event: unknown) => void
...
function handlers(_renderer: unknown, id = "trigger") {
  return {
    root: capturedNode(captures.root, id),
    trigger: capturedNode(captures.trigger, id),
  }
}
...
describe("ContextMenu native gesture bypass", () => {
  ...
  beforeEach(() => {
    ...
    captures.root.mockClear()
    captures.trigger.mockClear()
  ...
  it("routes keyboard activation through contextmenu without overriding caller cancellation", async () => {
    ...
    const renderer = render(triggerTree())
  ...
  it("keeps a different trigger root enabled while another root is armed", async () => {
    ...
    const roots = [capturedNode(captures.root, "first"), capturedNode(captures.root, "second")]
    const triggers = [capturedNode(captures.trigger, "first"), capturedNode(captures.trigger, "second")]
    const firstSource = {}
    await act(async () => {
  ...
  it("never overrides caller disabled ownership", async () => {
    policy.disposition.mockReturnValue("native")
    const renderer = render(triggerTree(true))
    const source = {}
    await act(async () => {
      handlers(renderer).trigger.props.onPointerDownCapture(syntheticPointer(pointerEvent(), source))
    })
    ...
    renderer.rerender(triggerTree(false))
    expect(handlers(renderer).root.props.disabled).toBe(false)
    renderer.unmount()
  })
