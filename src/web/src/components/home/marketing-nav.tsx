"use client";

import Image from "next/image";
import Link from "next/link";
import { useEffect, useState } from "react";

export function MarketingNav({
  isLoggedIn,
  showTemplates = true,
  ctaLabel,
  homeHref = "/",
  revealAfterHero = false,
  collapseLinksOnMobile = false,
  highlightActions = false,
}: {
  isLoggedIn: boolean;
  showTemplates?: boolean;
  ctaLabel?: string;
  homeHref?: string;
  revealAfterHero?: boolean;
  collapseLinksOnMobile?: boolean;
  highlightActions?: boolean;
}) {
  const [revealed, setRevealed] = useState(false);

  useEffect(() => {
    if (!revealAfterHero) return;

    const update = () => {
      const hero = document.querySelector(".hero-section");
      setRevealed(Boolean(hero && hero.getBoundingClientRect().bottom <= 0));
    };

    update();
    window.addEventListener("scroll", update, { passive: true });
    return () => window.removeEventListener("scroll", update);
  }, [revealAfterHero]);

  return (
    <nav
      className={`marketing-nav fixed top-0 right-0 left-0 z-50 transition-opacity duration-300 ${revealAfterHero && revealed ? "visible opacity-100" : "invisible opacity-0"}`}
      style={{
        backgroundColor: "var(--landing-bg-translucent)",
        backdropFilter: "blur(8px)",
        borderBottom: "1px solid var(--landing-border)",
      }}
    >
      <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-2">
        <Link href={homeHref} className="flex items-center gap-1">
          <Image src="/alook.svg" alt="Alook" width={22} height={22} />
          <span
            className="text-lg tracking-tight"
            style={{
              fontFamily: "var(--font-brand)",
              color: "var(--landing-text)",
              fontWeight: 700,
            }}
          >
            Alook
          </span>
        </Link>

        <div className="relative flex items-center gap-3">
          {showTemplates && (
            <Link
              href="/templates"
              className="hidden sm:block px-3 py-2 text-xs uppercase tracking-widest transition-opacity duration-150 hover:opacity-70"
              style={{
                fontFamily: "var(--font-mono)",
                color: "var(--landing-text)",
              }}
            >
              Templates
            </Link>
          )}
          <a
            href="/blog"
            className="hidden sm:block px-3 py-2 text-xs uppercase tracking-widest transition-opacity duration-150 hover:opacity-70"
            style={{
              fontFamily: "var(--font-mono)",
              color: "var(--landing-text)",
            }}
          >
            Blog
          </a>
          <a
            href="https://github.com/alookai/alook"
            target="_blank"
            rel="noopener noreferrer"
            className={`${collapseLinksOnMobile ? "hidden sm:flex" : "flex"} items-center justify-center p-2 transition-opacity duration-150 hover:opacity-70`}
            aria-label="GitHub"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill={highlightActions ? "var(--landing-accent)" : "var(--landing-text)"}>
              <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
            </svg>
          </a>
          <a
            href="https://discord.alook.ai"
            target="_blank"
            rel="noopener noreferrer"
            className={`${collapseLinksOnMobile ? "hidden sm:flex" : "flex"} items-center justify-center p-2 transition-opacity duration-150 hover:opacity-70`}
            aria-label="Discord"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill={highlightActions ? "var(--landing-accent)" : "var(--landing-text)"}>
              <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z" />
            </svg>
          </a>
          <a
            href="https://x.com/alook_ai"
            target="_blank"
            rel="noopener noreferrer"
            className={`${collapseLinksOnMobile ? "hidden sm:flex" : "flex"} items-center justify-center p-2 transition-opacity duration-150 hover:opacity-70`}
            aria-label="Follow us on X"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill={highlightActions ? "var(--landing-accent)" : "var(--landing-text)"}>
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
          </a>
          {collapseLinksOnMobile && (
            <details className="group relative sm:hidden">
              <summary
                className="flex size-9 cursor-pointer list-none items-center justify-center border transition-opacity duration-150 hover:opacity-70 [&::-webkit-details-marker]:hidden"
                style={{
                  color: highlightActions ? "var(--landing-accent)" : "var(--landing-text)",
                  borderColor: highlightActions ? "var(--landing-accent)" : "var(--landing-border)",
                }}
                aria-label="Open navigation menu"
              >
                <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <circle cx="5" cy="12" r="1.7" />
                  <circle cx="12" cy="12" r="1.7" />
                  <circle cx="19" cy="12" r="1.7" />
                </svg>
              </summary>
              <div
                className="absolute right-0 top-full mt-2 min-w-40 border p-2 shadow-lg"
                style={{
                  backgroundColor: "var(--landing-bg)",
                  borderColor: "var(--landing-border)",
                }}
              >
                {showTemplates && <Link href="/templates" className="block px-3 py-2 text-xs uppercase tracking-widest">Templates</Link>}
                <a href="/blog" className="block px-3 py-2 text-xs uppercase tracking-widest">Blog</a>
                <a href="https://github.com/alookai/alook" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 text-xs uppercase tracking-widest">GitHub</a>
                <a href="https://discord.alook.ai" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 text-xs uppercase tracking-widest">Discord</a>
                <a href="https://x.com/alook_ai" target="_blank" rel="noopener noreferrer" className="block px-3 py-2 text-xs uppercase tracking-widest">X</a>
              </div>
            </details>
          )}
          {isLoggedIn ? (
            <Link
              href="/c/me"
              className="inline-flex items-center gap-2 px-4 py-2 text-xs uppercase tracking-widest transition-opacity duration-150 hover:opacity-70"
              style={{
                fontFamily: "var(--font-mono)",
                color: highlightActions ? "var(--landing-bg)" : "var(--landing-text)",
                backgroundColor: highlightActions ? "var(--landing-accent)" : "transparent",
                border: `1px solid ${highlightActions ? "var(--landing-accent)" : "var(--landing-text)"}`,
              }}
            >
              {ctaLabel ?? "App"}
              <svg
                width="12"
                height="12"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                strokeWidth="2.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              >
                <path d="M5 12h14M12 5l7 7-7 7" />
              </svg>
            </Link>
          ) : (
            <a
              href="/sign-in"
              className="inline-flex items-center gap-2 px-4 py-2 text-xs uppercase tracking-widest transition-opacity duration-150 hover:opacity-70"
              style={{
                fontFamily: "var(--font-mono)",
                color: "var(--landing-bg)",
                backgroundColor: highlightActions ? "var(--landing-accent)" : "var(--landing-text)",
              }}
            >
              {ctaLabel ?? "Get Started"}
            </a>
          )}
        </div>
      </div>
    </nav>
  );
}
