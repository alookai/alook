...
export function WorkspaceShell({ children }: { children: ReactNode }) {
    ...
    return (
      <SidebarTriggerContext.Provider value={() => setSidebarOpen(true)}>
        <AgentChatSheetProvider>
          <div ref={shellRef} className="workspace-shell flex flex-col h-dvh overflow-hidden relative">
            <AppBackground />
  ...
  return (
    <AgentChatSheetProvider>
      <div ref={shellRef} className="workspace-shell flex h-dvh overflow-hidden relative">
        <AppBackground />
        <AppSidebar />
