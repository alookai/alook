...
import { MessageBody } from "@/components/community/messages/message-body"
import { BotApprovalCard } from "@/components/community/social/bot-approval-card"
import { EmojiPickerPopover } from "@/components/community/messages/emoji-picker"
import { Tooltip, TooltipTrigger, TooltipContent } from "@/components/ui/tooltip"
import { NumberTicker } from "@/components/ui/number-ticker"
import { MessageContextItems, MessageDropdownItems, hasMessageMenu } from "@/components/community/messages/message-menu"
...
import { AttachmentCard } from "@/components/community/messages/attachment-card"
...
function MessageRowViewImpl(props: MessageRowViewProps) {
  return renderMessageRowView(props)
}
...
export type MessageRowViewProps = {
  ...
  touchMenuAnchor?: ReturnType<typeof createMessageMenuPointAnchor> | null
  onTouchMenuOpenChange?: (open: boolean) => void
  onTouchStart?: () => void
  onTouchEnd?: () => void
  onTouchCancel?: () => void
  onTouchBodyClick?: (event: React.MouseEvent<HTMLDivElement>) => void
  activated?: boolean
  onActivate?: () => void
}

export function renderMessageRowView({
  ...
  marked = false, toolbarOpen = false,
  onToolbarOpenChange = noop, onContextOpenChange = noop,
  touchMenuOpen = false, touchMenuAnchor = null,
  onTouchMenuOpenChange = noop, onTouchStart = noop, onTouchEnd = noop,
  onTouchCancel = noop, onTouchBodyClick = noop,
  activated = false, onActivate = noop,
}: MessageRowViewProps) {
  ...
  const activate = interactive && hoverCapable && !activated
    ? (event: React.SyntheticEvent<HTMLElement>) => {
        if (shouldActivateMessageOverlays(event.target)) onActivate()
      }
    : undefined
    ...
    >
      ...
      {interactive && activated && !selectMode && (
        <div className={`absolute right-2 z-20 flex items-center gap-1 rounded-lg border border-border/60 bg-card px-2 py-1 shadow-(--e1) transition-opacity duration-150 ${m.grouped ? "-top-2" : "-top-3"} ${toolbarOpen ? "opacity-100" : "pointer-events-none opacity-0 group-hover:pointer-events-auto group-hover:opacity-100"}`}>
          ...
          {onReact && (
            <EmojiPickerPopover side="bottom" align="end" onPick={(e) => onReact(e)} onOpenChange={onToolbarOpenChange}>
          ...
          <DropdownMenu onOpenChange={onToolbarOpenChange}>
            <DropdownMenuTrigger
  ...
  if (!hoverCapable) {
    return (
      <DropdownMenu open={touchMenuOpen} onOpenChange={onTouchMenuOpenChange}>
...
type MessageProps = Parameters<typeof MessageRowViewImpl>[0]
...
function messagePropsEqual(prev: MessageProps, next: MessageProps): boolean {
  ...
  return (
    ...
    prev.markedLoading === next.markedLoading &&
    prev.toolbarOpen === next.toolbarOpen &&
    prev.onToolbarOpenChange === next.onToolbarOpenChange &&
    prev.onContextOpenChange === next.onContextOpenChange &&
    prev.touchMenuOpen === next.touchMenuOpen &&
    prev.touchMenuAnchor === next.touchMenuAnchor &&
    prev.onTouchMenuOpenChange === next.onTouchMenuOpenChange &&
    prev.onTouchStart === next.onTouchStart &&
    prev.onTouchEnd === next.onTouchEnd &&
    prev.onTouchCancel === next.onTouchCancel &&
    prev.onTouchBodyClick === next.onTouchBodyClick &&
    prev.activated === next.activated &&
    prev.onActivate === next.onActivate
  )
}

export const MessageRowView = memo(MessageRowViewImpl, messagePropsEqual)
