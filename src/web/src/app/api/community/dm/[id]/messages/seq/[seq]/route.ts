 ...
 *
 * Resolve a seq number to its message ID within a DM. Used for message
...
export const GET = withAuth(async (req: NextRequest, ctx) => {
  ...
  if (typeof dmId !== "string" || typeof seqStr !== "string") {
  ...
  const message = await withD1Retry(
    () => queries.communityMessage.getMessageByChannelAndSeq(db, { channelId: dmId }, seq),
    { route: "dm/messages/seq" },
  )
  if (!message) {
