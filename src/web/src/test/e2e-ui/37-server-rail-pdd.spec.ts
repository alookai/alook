...
test("server rail keeps scroll separate from native, touch, and keyboard drag", async ({ asUser }, testInfo) => {
  ...
  const darkKeyboardSource = page.getByTestId(tid.serverIcon(scrollSwipeServer))
  await darkKeyboardSource.focus()
  await expect(
    darkKeyboardSource.locator("xpath=ancestor::*[@data-slot='context-menu-trigger'][1]"),
  ).toBeVisible()
  await darkKeyboardSource.focus()
  await expect(darkKeyboardSource).toBeFocused()
  await darkKeyboardSource.press("Space")
  await expect(darkKeyboardSource).toHaveAttribute("data-dragging", "true")
  await expect(darkKeyboardSource).toBeFocused()
  await page.keyboard.press("ArrowUp")
  const keyboardPreview = page.locator("[data-rail-preview]")
  await expect(keyboardPreview).toHaveCount(1)
  ...
  await expect(keyboardPreview).toHaveCount(0)
  await page.emulateMedia({ colorScheme: "light" })
  await expect(page.locator("html")).not.toHaveClass(/dark/)
  await firstFolder.scrollIntoViewIfNeeded()
  await expect(firstFolder).toBeInViewport()
  const pendingFolderTestId = await firstFolder.getAttribute("data-testid")
  expect(pendingFolderTestId).not.toBeNull()
  ...
  try {
    const keyboardSource = page.getByTestId(tid.serverIcon(scrollSwipeServer))
    await keyboardSource.focus()
    await expect(
      keyboardSource.locator("xpath=ancestor::*[@data-slot='context-menu-trigger'][1]"),
    ).toBeVisible()
    await keyboardSource.focus()
    await expect(keyboardSource).toBeFocused()
    await keyboardSource.press("Space")
    await expect(keyboardSource).toHaveAttribute("data-dragging", "true")
    await expect(keyboardSource).toBeFocused()
    await page.keyboard.press("ArrowUp")
    await expect(page.locator("[data-rail-preview]")).toHaveCount(1)
    await page.keyboard.press("Enter")
    await expect.poll(() => railRequests.length).toBe(9)

    const existingFolder = page.getByTestId(pendingFolderTestId!)
    await existingFolder.scrollIntoViewIfNeeded()
    await expect(existingFolder).toBeInViewport()
