...
// terminal call that resolves to rows. We don't re-verify the WHERE
// predicate itself here (that's SQL, exercised in e2e/integration); this
// covers the post-fetch `lastReadSeq < newSeq` filter and row shape, which
// is the part `enqueueBotWakes` actually depends on.
...
describe("findWakeCandidates", () => {
  it("returns [] and never queries when recipients is empty", async () => {
    ...
    const result = await findWakeCandidates(db as never, { recipients: [], channelId: "c1", newSeq: 5 });
