...
// A DM channel: no server, no parent (post-#412 DMs are channels, but a DM row
// has serverId null and parentChannelId null).
const DM = { id: "dm1", serverId: null as unknown as string, parentChannelId: null };
...
describe("computeEffectiveLevel — DM independence (Gener/Melly #4)", () => {
  it("a DM with no own setting → 'all' (self-contained, defaults to all)", () => {
    expect(notif.computeEffectiveLevel([], DM)).toBe("all");
  });

  it("a DM honors its OWN 'nothing' setting (active mute)", () => {
    const settings = [{ channelId: "dm1", serverId: null, level: "nothing" }];
    expect(notif.computeEffectiveLevel(settings, DM)).toBe("nothing");
  });

  it("a 'nothing' set on some OTHER server does NOT cascade onto a DM (no inheritance)", () => {
    // The DM's serverId is null, so a server-scoped setting can never match it.
    const settings = [{ channelId: null, serverId: "s1", level: "nothing" }];
    expect(notif.computeEffectiveLevel(settings, DM)).toBe("all");
  });

  it("a 'nothing' set on another channel does NOT touch a DM", () => {
    const settings = [{ channelId: "c1", serverId: null, level: "nothing" }];
    expect(notif.computeEffectiveLevel(settings, DM)).toBe("all");
  });
});
