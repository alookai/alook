...
function fakeHandle(isStreaming = false) {
  ...
  } as VendorSessionHandle & { isStreaming: boolean; prompt: ReturnType<typeof vi.fn>; steer: ReturnType<typeof vi.fn> };
}

describe("SdkLane.send", () => {
  it("resolves admission before the vendor prompt promise settles", async () => {
  ...
  // `logical session.start()`, which fires the very first turn this way)
