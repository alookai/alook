...
import { createPiSessionDependencies, loadPiSdkModule, type PiSdkModule, type PiSdkLoader } from "./sessionDeps.js";
import type { AdapterLaunchContext } from "../../internal/adapter.js";
import type { PiConfig } from "../../contract.js";
...
function baseCtx(overrides: Partial<AdapterLaunchContext> = {}): AdapterLaunchContext {
  return fakeLaunchContext("pi", mkTmp(), {
    prepared: fakePrepared({
      networkProtected: { ALOOK_PROXY_URL: "http://127.0.0.1:9/proxy" },
...
function piConfig(overrides: Partial<PiConfig> = {}): PiConfig {
  return {
    model: { kind: "default" },
...
describe("createPiSessionDependencies — buildSpawnEnv", () => {
  it("delegates to prepareCliTransport: credential voucher present, PATH includes the link dir", async () => {
    const ctx = baseCtx();
    const deps = createPiSessionDependencies(ctx, fakeSdk().loader);
...
describe("createPiSessionDependencies — createAgentSession", () => {
  it("sets the runtime API key when the runtime config carries a pi-builtin provider", async () => {
    const fake = fakeSdk();
    const ctx = baseCtx({
      config: { runtimeConfig: piConfig({ provider: { kind: "builtin", providerId: "google", apiKey: "key-123" } }) },
    });
    const deps = createPiSessionDependencies(ctx, fake.loader);
...
describe("loadPiSdkModule — global-install fallback", () => {
  it("falls back to resolving + import()-ing the real install dir when the bare specifier isn't resolvable (never true in this test env)", async () => {
    ...
    vi.resetModules();
    vi.doMock("./index.js", () => ({ resolvePiSdkPackageDir: () => pkgDir }));
    const fresh = await import("./sessionDeps.js");
  ...
  it("throws a clear error when pi isn't found on PATH either", async () => {
    vi.resetModules();
    vi.doMock("./index.js", () => ({ resolvePiSdkPackageDir: () => undefined }));
    const fresh = await import("./sessionDeps.js");
  ...
  it("retries from scratch after a failed load instead of replaying the same rejection forever", async () => {
    ...
    vi.doMock("./index.js", () => ({ resolvePiSdkPackageDir: () => (resolvable ? pkgDir : undefined) }));
    const fresh = await import("./sessionDeps.js");
