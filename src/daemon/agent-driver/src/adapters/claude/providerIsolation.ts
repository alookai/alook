...
export function buildClaudeProviderIsolationEnv(ctx: AdapterLaunchContext): NodeJS.ProcessEnv {
  const provider = ctx.config.runtimeConfig && "provider" in ctx.config.runtimeConfig
    ? ctx.config.runtimeConfig.provider
    : undefined;
  const hasCustomProvider = provider?.kind === "custom_endpoint";
