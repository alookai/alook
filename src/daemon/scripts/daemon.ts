 ...
 * flags work in dev (e.g. `pn daemon start --machine-key … --ws-url …`).
 * Community mode is supported by omitting `--server-url`.
 *
 * For the legacy alook flow, set ALOOK_MACHINE_KEY/ALOOK_SERVER_URL/
 * ALOOK_SERVER_WS_URL or pass the matching `--server-url`/`--ws-url` flags.
 */
...
program
  ...
  .option("--server-url <url>", "server HTTP URL (or ALOOK_SERVER_URL); omit for community mode", process.env.ALOOK_SERVER_URL)
