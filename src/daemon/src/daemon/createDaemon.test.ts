...
if (process.env.ALOOK_WINDOWS_JOB_DEBUG_FILE) appendFileSync(process.env.ALOOK_WINDOWS_JOB_DEBUG_FILE, "runtime-boot\\n");
    ...
    try {
      ...
      const admission = await new Promise<Awaited<typeof started>>((resolve, reject) => {
        const timer = setTimeout(() => {
          reject(new Error(
            `fresh Codex admission timed out; phases: ${existsSync(debugFile) ? JSON.stringify(readFileSync(debugFile, "utf8")) : "missing"}; raw stdout: ${JSON.stringify(rawLines)}; events: ${JSON.stringify(events)}`,
          ));
        }, 15_000);
        void started.then(
          (result) => {
